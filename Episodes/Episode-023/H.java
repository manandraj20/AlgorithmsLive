...
public class H
{
   public static void main(String[] args) throws Exception
   {
      ...
      int T = in.nextInt();
      for (int tc=1; tc<=T; tc++)// no of test cases
         new H(tc, in, out);// running tarjan algorithm on every test case
      out.close();//closing the stdout
   }
   ...
   void dfs(int i)
   {
      ...
      {System.out.println(stk);
         while (true)
         {
            ...
            if (i == j)// when you hit the node which started the scc then break
               break;
         }
         numSCCs++;
      }
